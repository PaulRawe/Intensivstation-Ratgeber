...
function insertSearchBox() {
    ...
    const searchBoxHTML = '<div class="search-box" id="auto-search-box"><h2>Frage suchen</h2>' +
...
(function () {
    ...
    function getQuickCheckHTML() {
        const css = getQuickCheckCSS();
        const html = getQuickCheckMarkup();
        return '<style>' + css + '</style>' + html;
    }

    function getQuickCheckCSS() {
        return '.qc-container{max-width:1200px;margin:0 auto 3rem;padding:0 20px}' +
            ...
            '.quick-check-question.active{display:block;animation:qcFadeInSlide .4s ease-out}' +
            '@keyframes qcFadeInSlide{0%{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}' +
            '.qc-question-text{font-size:1.15rem;font-weight:600;color:#2c3e50;margin-bottom:1.2rem;line-height:1.5}' +
            ...
            '.qc-answer-button{background:#fff;border:2px solid #e9ecef;padding:1rem 1.5rem;border-radius:8px;font-size:1rem;font-weight:500;color:#2c3e50;cursor:pointer;transition:all .3s ease;text-align:left;position:relative;font-family:inherit;width:100%}' +
            '.qc-answer-button:hover{border-color:#5dade2;background:#f0f9ff;transform:translateX(4px)}' +
            '.qc-answer-button::before{content:"→";position:absolute;right:1.5rem;opacity:0;transition:all .3s ease;color:#5dade2}' +
            '.qc-answer-button:hover::before{opacity:1;right:1rem}' +
            ...
            '.qc-progress-fill{background:linear-gradient(90deg,#5dade2,#3498db);height:100%;transition:width .4s ease;border-radius:3px}' +
            '.qc-results-section{display:none;text-align:center}' +
            '.qc-results-section.show{display:block;animation:qcFadeInSlide .5s ease-out}' +
            '.qc-results-header{margin-bottom:2rem}' +
            '.qc-results-icon{font-size:3rem;margin-bottom:1rem}' +
            ...
            '.qc-product-badge{position:absolute;top:-12px;right:20px;background:#ff6b35;color:#fff;padding:.4rem 1rem;border-radius:20px;font-size:.85rem;font-weight:600;box-shadow:0 2px 8px rgba(255,107,53,.3)}' +
            ...
            '.qc-product-button{background:linear-gradient(135deg,#5dade2,#3498db);color:#fff;padding:.9rem 2rem;border-radius:8px;text-decoration:none;font-weight:600;font-size:1rem;transition:all .3s ease;display:inline-block;box-shadow:0 4px 12px rgba(93,173,226,.3)}' +
            '.qc-product-button:hover{transform:translateY(-2px);box-shadow:0 6px 16px rgba(93,173,226,.4);color:#fff}' +
            '.qc-restart-button{margin-top:2rem;padding:.8rem 1.5rem;background:#fff;border:2px solid #5dade2;color:#5dade2;border-radius:8px;font-weight:600;cursor:pointer;transition:all .3s ease;font-family:inherit;font-size:1rem}' +
            '.qc-restart-button:hover{background:#5dade2;color:#fff}' +
            '@media (max-width:768px){.quick-check-wrapper{padding:1.5rem;margin:2rem 0 1.5rem}' +
            '.quick-check-header h3{font-size:1.3rem}' +
            '.qc-question-text{font-size:1.05rem}' +
            '.qc-answer-button{padding:.9rem 1.2rem;font-size:.95rem}' +
            '.qc-product-card{padding:1.5rem}' +
            '.qc-product-title{font-size:1.15rem}' +
            '.qc-product-footer{flex-direction:column;gap:1rem;align-items:stretch}' +
            '.qc-product-button{width:100%;text-align:center}}';
    }

    function getQuickCheckMarkup() {
        return '<div class="qc-container">' +
